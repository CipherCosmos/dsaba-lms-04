...
export const bulkUpdateCOTargets = createAsyncThunk(
  ...
  async ({ subjectId, coTargets }: { subjectId: number; coTargets: Partial<COTarget>[] }) => {
    const response = await coTargetAPI.bulkUpdate(subjectId, coTargets as any[])
    return response
  }
)
...
export const createIndirectAttainment = createAsyncThunk<IndirectAttainment, { subjectId: number; attainmentData: Partial<IndirectAttainment> }>(
  'copo/createIndirectAttainment',
  async ({ subjectId, attainmentData }) => {
    const response = await indirectAttainmentAPI.create(subjectId, attainmentData)
    return response as IndirectAttainment
  }
)

export const updateIndirectAttainment = createAsyncThunk<IndirectAttainment, { attainmentId: number; attainmentData: Partial<IndirectAttainment> }>(
  'copo/updateIndirectAttainment',
  async ({ attainmentId, attainmentData }) => {
    const response = await indirectAttainmentAPI.update(attainmentId, attainmentData)
    return response as IndirectAttainment
  }
)

export const deleteIndirectAttainment = createAsyncThunk<number, number>(
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchAssessmentWeights.fulfilled, (state, action) => {
        ...
        // API returns an object with max_internal and max_external, convert to AssessmentWeight array
        const payload = action.payload as any
        if (payload && (payload.max_internal !== undefined || payload.max_external !== undefined)) {
          // Create a single AssessmentWeight entry representing the subject's marks configuration
          state.assessmentWeights = [{
            id: 0,
            subject_id: state.selectedSubjectId || 0,
            exam_type: 'internal1',
            weight_pct: payload.max_internal || 0,
            created_at: new Date().toISOString(),
          }] as AssessmentWeight[]
        } else {
          state.assessmentWeights = Array.isArray(payload) ? payload : []
        }
      })
    ...
    builder
      ...
      .addCase(createIndirectAttainment.fulfilled, (state, action: any) => {
        const payload = action.payload as any
        if (payload) {
          state.indirectAttainment.push(payload as IndirectAttainment)
        }
      })
      .addCase(updateIndirectAttainment.fulfilled, (state, action: any) => {
        const payload = action.payload as any
        if (payload && payload.id) {
          const index = state.indirectAttainment.findIndex(ia => ia.id === payload.id)
          if (index !== -1) {
            state.indirectAttainment[index] = payload as IndirectAttainment
          }
        }
      })
      .addCase(deleteIndirectAttainment.fulfilled, (state, action: any) => {
        const payload = action.payload as any
        const id = typeof payload === 'number' ? payload : (payload?.id || payload)
        if (id) {
          state.indirectAttainment = state.indirectAttainment.filter(ia => ia.id !== id)
        }
      })
    ...
    builder
      ...
      .addCase(fetchAttainmentAudit.fulfilled, (state, action: any) => {
        state.attainmentAuditLoading = false
        const payload = action.payload as any
        state.attainmentAudit = Array.isArray(payload) ? payload : []
      })
    ...
    builder
      ...
      .addCase(fetchSubjectAttainment.fulfilled, (state, action: any) => {
        state.subjectAttainmentLoading = false
        const payload = action.payload as any
        state.subjectAttainment = payload || null
      })
