...
def upgrade():
    # Check if enum types already exist and create them if they don't
    from sqlalchemy import text
    
    # Create missing enum types (examtype already exists)
    conn = op.get_bind()
    ...
    op.create_table('po_definitions',
        ...
        sa.Column('title', sa.String(length=200), nullable=False),
        sa.Column('description', sa.Text(), nullable=True),
        sa.Column('type', sa.Enum('PO', 'PSO', name='potype'), nullable=False),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    ...
    op.create_table('assessment_weights',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('subject_id', sa.Integer(), nullable=False),
        sa.Column('exam_type', sa.Enum('internal1', 'internal2', 'final', name='examtype'), nullable=False),
...
def downgrade():
    ...
    # Drop custom enum types (only if not used by other tables)
    from sqlalchemy import text
